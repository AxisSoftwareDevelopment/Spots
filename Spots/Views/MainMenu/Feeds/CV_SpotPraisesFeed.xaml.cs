...
public partial class CV_SpotPraisesFeed : ContentView
{
    private readonly FeedContext<SpotPraise> CurrentFeedContext = new();
    public CV_SpotPraisesFeed()
	{
		InitializeComponent();
    ...
    {
        if (SessionManager.CurrentSession?.Spot != null)
        {
            return await DatabaseManager.FetchSpotPraises_FromSpot(SessionManager.CurrentSession.Spot, lastItemFetched);
        }
        else
        {
            return [];
